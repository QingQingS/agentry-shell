...
## 九、下次从这里继续
Opt 1 已实测验证（−49%）并提交。下一步 **Opt 2（index 交给代码）**：用同协议（`sh tests/reset_wiki.sh` → 跑 `ingest_single_topic.md` → 比 14435）测其增量。目标继续压"带 tool_call 的轮数"。另：curation run-to-run 不一致是独立"输出一致性"议题，性能理顺后单独治。
