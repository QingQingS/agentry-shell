...
## 十一、最后一轮对话记录
提问：关于Opt3 裁剪历史，每步都要保留reasoning_content的问题，我有一个新的想法，其实不需要做成显示的多轮对话，多轮对话历史记录存
  在的原因是让模型知道前面都发生了什么，我们这里完全可以采用每次都是首次对话的形式来进行模拟，只要每次给够模型它当前所处的状
  态即可，比如首次的环境是wiki_schema
  和index.md和要归档的文件，把它们喂给模型，让模型给出一个它的解决方案，它已经知道要归档的文件内容是什么样子了，实体有哪些，
  也知道跟哪些wiki相关或是否要进行创建新类别等，可以通过json的方式给出，比如下一步要写文件，写完文件后，环境状态更新了，可以
  通过更新json内容来实现，然后把新环境状态给模型，而不是把历史对话记录给模型，让模型根据新环境来判断下一步动作，大概有这么个
  想法，不知道我是否表达清楚了，当然中间的细节还为想清楚，我们可以通过讨论来一起完善，看是否可行
回答：⏺ 你表达得很清楚,而且这是个好想法——它触到了问题的根上。我先把它复述确认,再讲我的判断(包括为什么我认为它特别对路),然后摆出需要
  一起想清楚的几个关键点。

  你的想法,我的理解

  把 ReAct 的「对话历史累积」换成「显式状态传递」:每次调用都是"第一次对话"——喂给模型 schema + 当前世界状态(catalog、待归档文
  档、已抽取的实体、相关性判断、已写了哪些页…),让它只产出下一步动作(JSON);代码执行动作后更新状态
  JSON,再用新状态发起又一次全新调用。模型从不看历史,只看"现在长什么样"。

  为什么我认为这条路很对

  1. 它从根上溶解了 400 问题。 那个约束只在「多轮 tool-calling 对话续接」时成立。如果每次调用都是独立单发(历史里没有上一轮的
  assistant-tool-call),就没有需要回传的 reasoning_content,400 根本不会触发。Opt 3 想"删历史思考"被 API
  拒;你这个是"压根不产生历史"——绕过而非对抗。

  2. 它精准打在我们实测到的成本大头上。 还记得 Musk 那次:input 32k vs output 3.7k——钱几乎全烧在反复重发的输入上。ReAct 的
  input 是 O(N²)(第 N 步要重发前 N-1 步的全部思考);状态传递是
  O(N)(每步只发当前状态,大小基本恒定)。步数越多、任务越复杂,差距越大。

  3. 它其实正是 Opt 3 想要的东西,只是换了实现。 Opt 3 的直觉是"结论是耐用的、啰嗦的推导是脚手架,该丢"。API
  不让你从历史里删。但你这个用结构换历史:把模型的结论(实体、页面计划、归属判断)固化进
  state,推导过程每步用完即弃——结构上天然做到了。这点我很喜欢。

  4. 它没有背离 agentic。 项目当初坚持"真 agentic"是因为"执行序列依赖运行时发现的信息"。你这个仍然是模型每步按当前状态自主决
  定下一步——只是记忆机制从"对话稿"变成"可检视的状态对象"。某种意义上更干净:模型变成一个纯函数 (state) →
  next_action,状态由代码维护。

  需要一起想清楚的几个点(crux)

  A. state 里到底装什么? 这是成败关键。至少要有:待归档文档(写页时需要)、抽取的实体/主题、当前
  catalog、计划(要建/改哪些页)、进度(哪些做完了)。难点:
  - 计划要冻结进 state。否则每步重新推理可能"改主意"(这步说建 People,下步又想归 AI)——这正是无历史的最大风险。把第 1
  步的决策固化下来,后续步骤是"执行计划"而非"重新规划"。
  - 更新已有页的情况:模型要合并就得看那页当前全文 → 那一步的 state 必须带上该页内容。所以 state 会随"要不要读某页"动态增减。

  B. 用原生 tool-calling 还是纯 JSON? 我倾向一个折中:原生 tool-calling 但无状态——每次调用 = system(schema) + user(当前 state)
   + tools=,模型回一个 tool_call,代码执行、更新 state、下一次全新调用(不把上一轮 append 进历史)。这样既拿到 tool-calling
  的结构化参数可靠性(大段 page content 当 JSON 字符串塞进自由文本容易坏),又因为"历史里没有上一轮 assistant 轮"而不触发
  400。比纯 JSON 稳。

  C. 收益在哪、不在哪。 简单单页归档我们现在已经很省了(7943 / 2
  步,几乎是下限),这个改造对它提升有限。真正的大赢在复杂多页/要更新已有页的场景(就是 Musk 那种 8 步 75382 的)——那里 ReAct 的
  O(N²) 才是灾难。所以验证要用复杂 fixture,别用现在那个简单的。

  老实说的代价

  - 这是 WikiAgent 循环的一次较大重写,不是微调。我们刚到 −72% 的干净状态,这是更根本的一层。
  - state 设计是全部难度所在——装少了模型瞎判断,装多了又不省。
  - 可靠性:模型要稳定按 schema 给结构化动作 + 正确发"完成"信号。

  我的建议

  想法可行、且方向比继续抠 ReAct 更有上限。建议下一步一起把 state
  的结构(schema)设计出来——特别是"计划冻结"和"更新已有页时怎么带页内容"这两块定了,整个机制就立住了。要不要我先起草一版 state
  JSON 的结构草案,我们对着它推敲?另外这事值不值得记进 wiki_agent优化.md 作为"Opt 4 候选方案"存档?