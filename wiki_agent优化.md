...
## 八、下次从这里继续
测量地基已立。下一步：在稳定 fixture 上测 Opt 1 的真实 delta（理想做 old-SCHEMA vs new-SCHEMA 各复位跑一次对比），确认后提交 Opt 1；再推进 Opt 2（index 交给代码）。另：curation run-to-run 不一致是独立的"输出一致性"议题，性能这条理顺后再单独治。
